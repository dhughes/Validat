...
<cfcomponent
	displayname="validatePrefix"
	output="false"
	hint="Data element prefix name validation rule."
	extends="validator">
	
	<!--- ------------------------------------------------------------ --->
	<!--- constructor --->

	<cffunction name="init" access="public" returntype="validatePrefix"
		hint="The default constructor for the validator rule, returning the initialized validator rule instance">

		<!--- call the base constructor --->
		<cfset super.init() />
		
		<!--- setup the list of accepted prefixes ... these might come from a database or config file as well --->
		<cfset variables.instance.prefixes = "Mr.,Mrs.,Ms.,Dr." />
	...
	<cffunction name="validate" access="public" output="false" returntype="boolean"
		hint="Check to see if data value is a valid prefix name.">

		<cfargument name="dataValue" type="string" required="true" hint="The data value to be validated" />
		
		<!--- check to see if data value is a valid prefix name. --->
		<cfif listFindNoCase( variables.instance.prefixes, arguments.dataValue ) >
			<cfreturn true />
		</cfif>
